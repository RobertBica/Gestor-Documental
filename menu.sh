...
USUARIOS_DAW=(1DAW)
USUARIOS_ESPECIALES=(alumnado profesorado administrador)
...
fntSeleccionarEtapaCurso() {
  ...
  # Si no tienes permisos de entrada, -d puede fallar y parecer "no existe".
  if [[ ! -d "$CURRENT" ]]; then
...
fntCrearUsuariosGrupos() {
  ...
  echo "Creando usuarios de cursos (si no existen)..."
  ...
  echo "Creando usuarios especiales (sin grupo principal)..."
  for u in "${USUARIOS_ESPECIALES[@]}"; do id "$u" &>/dev/null || useradd -m "$u"; done

  # Asignar privilegios de administrador
  usermod -aG sudo administrador 2>/dev/null
...
fntBorrarUsuariosGrupos() {
  ...
  for u in "${USUARIOS_ESO[@]}" "${USUARIOS_BACH[@]}" "${USUARIOS_DAW[@]}" "${USUARIOS_ESPECIALES[@]}"; do
